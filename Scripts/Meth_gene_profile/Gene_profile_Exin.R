...
res_body <- ddply(subData, c("Segment_number", "Segment_type","Individual", "Exin_annotation", "CGI_prom"), function(x) c(mean(x$CvP_DMR), mean(x$Meth.CvP, na.rm=T), mean(x$Mean_per_dinuc, na.rm=T), mean(x$non_CpG_Fst, na.rm=T), mean(x$CpG_Fst, na.rm=T), cor(x$Mean_per_dinuc, log10(x$Expression_level+1), method="spearman", use = "complete.obs" )))
...
colnames(all.res) <- c("Segment_number", "Segment_type","Individual", "CGI_prom","DMR_frequency", "Meth.CvP","Meth", "non_CpG_Fst", "CpG_Fst", "Cor", "Exin_annotation")
all.res$Segment_number <- as.integer(all.res$Segment_number)

save(all.res, file = paste(args[2], args[3],  "Exin.rda", sep="_" ))
